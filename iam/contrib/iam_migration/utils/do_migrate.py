...
class Client(object):
    ...
    # ------------------ add common_actions
    def api_batch_add_common_actions(self, system_id, data):
        path = "/api/v1/model/systems/{system_id}/configs/common_actions".format(system_id=system_id)
        ok, message, data = self._call_iam_api(http_post, path, data)
        return ok, message

    def api_batch_update_common_actions(self, system_id, data):
        path = "/api/v1/model/systems/{system_id}/configs/common_actions".format(system_id=system_id)
        ok, message, data = self._call_iam_api(http_put, path, data)
        return ok, message
    ...
    def batch_update_common_actions(self, system_id, data):
        """通过更新直接注册常用信息"""
        return self.api_batch_update_common_actions(system_id, data)
